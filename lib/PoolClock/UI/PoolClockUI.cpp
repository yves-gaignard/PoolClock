/**
 * \file PoolClockUI.cpp
 * \author Yves Gaignard
 * \brief Implementation of member function of PoolClockUI 
 */

#include "PoolClockUI.h"
...
PoolClockUI* PoolClockUI::instance = nullptr;
PoolClockUI* ClockUI = PoolClockUI::getInstance();
...
/**
 * \brief Either instantiate a new PoolClockUI object by calling the private constructor and return it's address
 *        or it an instance of it already exists just return that.
 *
 * \return PoolClockUI* address of the new/already existing blynk config object
 */
PoolClockUI* PoolClockUI::getInstance()
{
	if(instance == nullptr)
	{
		instance = new PoolClockUI();
	}
	return instance;
}
 ...
 *
 */
void PoolClockUI::stop()
{
	vTaskDelete(PoolClockUILoop);
}
 ...
 *
 */
void PoolClockUI::changeSelection(ColorSelector selector, bool state)
{
	if(state == true)
	{
	...
	if(ClockUI->ColorSelection == PoolClockUI::CHANGE_DOT_COLOR)
	{
 ...
 *
 */
void PoolClockUICode(void* pvParameters)
{
 ...
 *
 */
void PoolClockUI::setup()
{
	Serial.println("Starting PoolClockUI on core 0...");
	//Setup the loop task on the second core
	xTaskCreatePinnedToCore(
	PoolClockUICode,	// Task function.
	"PoolClockUILoop",	// name of task.
	10000,			    // Stack size of task
	NULL,		    	// parameter of the task
	1,				    // priority of the task
	&PoolClockUILoop,	// Task handle to keep track of created task
...
BLYNK_WRITE(BLYNK_CHANNEL_LIGHT_GROUP_SELECTOR)
{
	...
	{
	case 1:
		ClockUI->ColorSelection = PoolClockUI::CHANGE_HOURS_COLOR;
		Blynk.virtualWrite(BLYNK_CHANNEL_CURRENT_COLOR_PICKER, ClockUI->HourColor.r, ClockUI->HourColor.g, ClockUI->HourColor.b);
		break;
	case 2:
		ClockUI->ColorSelection = PoolClockUI::CHANGE_MINUTES_COLOR;
		Blynk.virtualWrite(BLYNK_CHANNEL_CURRENT_COLOR_PICKER, ClockUI->MinuteColor.r, ClockUI->MinuteColor.g, ClockUI->MinuteColor.b);
		break;
	case 3:
		ClockUI->ColorSelection = PoolClockUI::CHANGE_INTERIOR_COLOR;
		Blynk.virtualWrite(BLYNK_CHANNEL_CURRENT_COLOR_PICKER, ClockUI->InternalColor.r, ClockUI->InternalColor.g, ClockUI->InternalColor.b);
		break;
	case 4:
		ClockUI->ColorSelection = PoolClockUI::CHANGE_DOT_COLOR;
		Blynk.virtualWrite(BLYNK_CHANNEL_CURRENT_COLOR_PICKER, ClockUI->DotColor.r, ClockUI->DotColor.g, ClockUI->DotColor.b);
		break;
	}
}
...
{
	ClockUI->changeSelection(PoolClockUI::CHANGE_HOURS_COLOR, param.asInt());
}

BLYNK_WRITE(BLYNK_CHANNEL_SELECTOR_MINUTES)
{
	ClockUI->changeSelection(PoolClockUI::CHANGE_MINUTES_COLOR, param.asInt());
}

BLYNK_WRITE(BLYNK_CHANNEL_SELECTOR_INTERIOR)
{
	ClockUI->changeSelection(PoolClockUI::CHANGE_INTERIOR_COLOR, param.asInt());
}

BLYNK_WRITE(BLYNK_CHANNEL_SELECTOR_DOT)
{
	ClockUI->changeSelection(PoolClockUI::CHANGE_DOT_COLOR, param.asInt());
}
